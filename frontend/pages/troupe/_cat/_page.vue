        ...
        <div class="single-troupe__header">
          <div class="container">
              ...
              <div class="archive__breadcrumb">
                <p><nuxt-link to="/troupe">Troupe</nuxt-link><span>></span>{{ this.name }}</p>
              </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async asyncData() {
      ...
      const theRelated = await fetch(
          this.$store.state.apiroute.url + '/api/troupes' + '?populate=deep,5',
        ).then((res) => {
        
        // can set up 404 redirection here
        return res.json();
      }); 
      this.relBlog = theRelated
      console.log('data', theRelated)
      console.log('relwork',this.relBlog)
  ...
  mounted(){
    this.asyncData();
        ...
        gsap.utils.toArray(".single-troupe__hero, .text-double").forEach((element, i) => {
        ...
        ScrollTrigger.batch(".single-troupe__hero, .text-double", {
            onEnter: batch => gsap.to(batch, {opacity: 1, y: 0, stagger: {each: 0.15, grid: [1, 3]}, overwrite: true}),
            onLeaveBack: batch => gsap.set(batch, {opacity: 0, y: 100, overwrite: true})
        });
        ScrollTrigger.addEventListener("refreshInit", () => gsap.set(".single-troupe__hero, .text-double", {y: 0}));
    },
}
</script>
